...
def interval_and_average(record, ten=True,fifty=True, all_avg=False):
    ...
    for i in range(0, len(record.Q_points)):
        ...
        p_R_P = sub_signal_interval(record.time,int(record.R_pos_properites["offset"][i]),int(record.P_points_properites["onset"][i][0]),0)
        pre_R_P.append(p_R_P)

        p_R_t = sub_signal_interval(record.time,int(record.T_points_properites["offset"][i][0]),int(record.R_pos_properites["onset"][i]),0)
        post_R_T.append(p_R_t)
